...
import (
	"go_web_method_post_get_gunain_form/entity"
	"html/template"
	"log"
	"net/http"
	"path"
...
//func input process
func InputProcessHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		//dapetin variabel yg di post
		err := r.ParseForm()
		if err != nil {
			log.Println(err)
			http.Error(w, "Error is happening, keep calm bro / sis", http.StatusInternalServerError)
